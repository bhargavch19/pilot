...
# Runs on Stop hook. Never blocks (warning only — exit 0).
#
# Per-repo overrides: add `.pilot.yml` or `.pilot.json` at the repo root
# with extra runner regexes:
#   # .pilot.yml
#   test_patterns:
#     - 'rake test'
#     - 'my-custom-runner'
...
if [[ -f .pilot.json ]]; then
  ...
  ' .pilot.yml 2>/dev/null | paste -sd'|' - 2>/dev/null || true)
fi
