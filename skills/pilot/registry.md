...
| 0.9 Map (knowledge graph) | "graphify"; "knowledge graph"; "map this folder"; "what's in here"; "connect these notes/papers"; "understand the structure"; a `/raw`-style drop folder | `graphify` | `claude-mem:learn-codebase`, `gsd-map-codebase`, `claude-mem:pathfinder` | builds a persistent, multimodal knowledge graph (code/PDFs/markdown/images → `graph.json` + report) from any folder; literal `/graphify` always routes here; prefer before deep exploration of an unfamiliar repo or a mixed-input notes folder. Distinct from `gsd-graphify` (GSD's own graph tool) |
